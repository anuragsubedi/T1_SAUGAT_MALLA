...
class Ui_MainWindow2(object):
    def setup2(self, MainWindow):
        ...
        self.label_2.setText("Original Value")
        self.label_3.setText("Quantity")
        self.label_4.setText("Disposal date")
        self.label_5.setText("Expiry")
        self.label_6.setText("Value After disposal")
    ...
    def inser(self):
        ...
        print(a1)
        if (a1 != ''):
          curr.execute('select * from stock where item = ?',(a1,))
          print('0')
          rows=curr.fetchall()
          conn.commit()
          print(rows)
          ...
          if len(rows)==0:
               ...
               curr.execute('Insert into stock values(?,?,?,?,?,?)',(self.lineEdit.text(),int(b),int(c),self.dateEdit.date().toPyDate(),self.dateEdit_2.date().toPyDate(),int(d)))
               print('B')
               conn.commit()
               a=0
               ...
               print(r1)
               curr2.execute('Insert into revenue values(?,?,?,?,?,?)',(self.lineEdit.text(),int(b),int(d),a,a,r1))
               conn2.commit()
               a= "insert sucessful"+"\nName: "+self.lineEdit.text()+"\nOriginal Value: "+b+"\nQuantity:"+c+"\nDisposal date:"+str(self.dateEdit.date().toPyDate())+"\nExpiry:"+str(self.dateEdit_2.date().toPyDate())+"\nValue After disposal: "+d
            ...
            if d!=0 and b!=0 and c!=0:
               ...
               curr.execute('Update stock set quantity = quantity+?, Original Value= ?, Value After disposal= ? where item = ?',(c,b,d,a1))
               conn.commit()
            elif c!=0 and b!=0:
                curr.execute('Update stock set quantity = quantity+?, Original Value= ? where item = ?',(c,b,a1))
                conn.commit()
            elif c!=0 and d!=0:
                curr.execute('Update stock set quantity = quantity+?, Value After disposal= ? where item = ?',(c,d,a1))
                conn.commit()
            elif  b!=0 and d!=0:
                curr.execute('Update stock set Value After disposal = ?, Original Value= ? where item = ?',(d,b,a1))
                conn.commit()
            elif b!=0:
                curr.execute('Update stock set Original Value= ? where item = ?',(b,a1))
                conn.commit()
            elif c!=0:
                curr.execute('Update stock set quantity = quantity+? where item = ?',(c,a1))
                conn.commit()
            elif d!=0:
                curr.execute('Update stock set Value After disposal= ? where item = ?',(d,a1))
                conn.commit()
            print('Done')
            curr.execute('select * from stock where item=?',(a1,))
            rowes=curr.fetchall()
            conn.commit()
            row=rowes[0]
            a= "update sucessful"+"\nName: "+row[0]+"\nOriginal Value: "+str(row[1])+"\nQuantity:"+str(row[2])+"\nDisposal date:"+row[3]+"\nExpiry:"+row[4]+"\nValue After disposal: "+str(row[5])
          print(a)
        else:
            a="Please insert appropriate data"
